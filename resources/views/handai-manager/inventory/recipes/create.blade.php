...
@vite('resources/css/handai-manager-inventory-recipes-create.css')
@endpush
    ...
    <form action="{{ route('manager.inventory.recipes.store') }}" method="POST" @submit.prevent="submitForm">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-4" x-show="selectedProductId && outputType !== 'semi'">
            ...
            <div class="overflow-x-auto -mx-5 px-5">
                ...
                <div x-show="ingredients.filter(i => i.type === 'semi_finished').length === 0" class="py-6 text-center text-sm text-gray-400">Belum ada.</div>
            </div>
        </div>
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <select x-model="newStock.category_id" class="rcp-select">
                        <option value="">-- Pilih --</option>
                        @foreach(\App\Models\StockCategory::all() as $cat)
                            <option value="{{ $cat->id }}">{{ $cat->name ?? $cat->stock_category_name ?? $cat->category_name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
