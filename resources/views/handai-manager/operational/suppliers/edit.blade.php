...
<div class="py-5 px-4 sm:px-6 lg:px-8 max-w-3xl mx-auto">
    ...
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500">Perbarui informasi {{ $supplier->name }}</p>
        </div>
    </div>

    <form action="{{ route('manager.operational.suppliers.update', $supplier) }}" method="POST"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="address" rows="2"
                          class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">{{ old('address', $supplier->address) }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Terms Pembayaran</label>
                <select name="payment_terms"
                        class="w-full h-10 px-3 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">
                    @foreach(['COD' => 'COD', 'NET7' => 'NET 7', 'NET14' => 'NET 14', 'NET30' => 'NET 30'] as $val => $label)
                    <option value="{{ $val }}" {{ old('payment_terms', $supplier->payment_terms) === $val ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" rows="2"
                          class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500">{{ old('notes', $supplier->notes) }}</textarea>
            </div>
        </div>
