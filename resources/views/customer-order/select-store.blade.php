            ...
            <div class="relative bg-white shadow-lg rounded-lg z-10">
                <div class="p-8">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-gray-500">Pilih toko yang tersedia untuk memulai pemesanan</p>
                    </div>

                    <!-- Pilihan Toko -->
                    <div class="flex justify-center space-x-4 mt-6">
                        <form action="{{ route('customerOrder.setStore') }}" method="POST"
                            class="flex flex-wrap justify-center gap-4">
                            @csrf
                            <input type="hidden" name="reseller_code" value="{{ $resellerCode }}">
                            
                            @foreach ($stores as $store)
                                <button type="submit" name="store_id" value="{{ $store->id }}"
                                    class="btn btn-dash btn-primary flex flex-col items-center justify-center h-32 w-28 text-center rounded-xl">
                                    <i class="ti ti-building-store text-xl mb-1"></i>
