...
@push('styles')
<style>
    /* only cloak required, inputs styled inline */
    [x-cloak] { display: none !important; }
</style>
@endpush

@section('content')
<div class="py-5 px-4 sm:px-6 lg:px-8 max-w-[900px] mx-auto" x-data="semiFinishedEdit()" x-cloak>
    ...
    <div class="flex items-center gap-3 mb-5">
        ...
        <div>
            <h1 class="text-lg font-semibold text-gray-800">Edit: {{ $sfp->name }}</h1>
            <p class="text-[12px] text-gray-400 mt-0.5">Perbarui definisi dan resep produk setengah jadi</p>
        </div>
    </div>
    ...
    <form action="{{ route('manager.inventory.semi-finished.update', $sfp->id) }}" method="POST" @submit.prevent="submitForm">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 mb-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="min_stock" x-model="form.min_stock" step="0.001" min="0" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Masa Expired (hari)</label>
                    <input type="number" name="expired_duration" x-model="form.expired_duration" step="1" min="0" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none">
                </div>
                <div class="md:col-span-2">
        ...
        {{-- Resep & perhitungan HPP kini dikelola melalui halaman Resep. --}}
        <div class="bg-yellow-50 border-l-4 border-yellow-300 p-4 mb-4">
            <p class="text-sm text-yellow-700">
                Untuk mengatur komposisi bahan dan menghitung HPP, buka <a href="{{ route('manager.inventory.recipes.index') }}" class="underline font-semibold">halaman Resep</a>.
            </p>
        </div>
...
@push('scripts')
<script>
function semiFinishedEdit() {
    return {
        form: {
            name: @json($sfp->name),
            unit_id: @json((string)$sfp->unit_id),
            min_stock: @json((float)$sfp->min_stock),
            expired_duration: @json($sfp->expired_duration ?? ''),
            description: @json($sfp->description ?? ''),
        },
        submitForm() {
