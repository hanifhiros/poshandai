...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-6" x-data="{ payModal: false, payId: null, payMax: 0, payDesc: '' }">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <a href="{{ route('manager.finance.ar.create') }}"
           class="inline-flex items-center gap-2 px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition">
            <i class="ti ti-plus text-base"></i> Tambah Piutang
        </a>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="fc p-4">
            <p class="text-[10px] uppercase tracking-wide text-gray-400 font-semibold mb-1">Total Piutang</p>
            <p class="text-lg font-bold text-gray-800">Rp{{ number_format($summary->total_amount, 0, ',', '.') }}</p>
        </div>
        <div class="fc p-4">
            <p class="text-[10px] uppercase tracking-wide text-gray-400 font-semibold mb-1">Sudah Diterima</p>
        ...
        <div class="fc p-4">
            ...
            <p class="text-lg font-bold text-red-600">{{ $overdueCount }} piutang</p>
        </div>
    </div>

    {{-- Filter --}}
    <div class="fc p-4 mb-6">
        <form method="GET" action="{{ route('manager.finance.ar.index') }}" class="flex flex-wrap items-end gap-3">
            <div>
            ...
            <div>
                ...
                <select name="customer" class="mt-1 block w-full text-sm border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500">
                    <option value="">Semua</option>
                    @foreach ($customers as $cust)
                        <option value="{{ $cust->id }}" {{ request('customer') == $cust->id ? 'selected' : '' }}>{{ $cust->name }}</option>
                    @endforeach
                </select>
            </div>
            <button type="submit" class="px-4 py-2 bg-gray-800 text-white text-sm rounded-lg hover:bg-gray-900 transition">Filter</button>
            <a href="{{ route('manager.finance.ar.index') }}" class="px-4 py-2 text-sm text-gray-600 hover:text-gray-800">Reset</a>
        </form>
    </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 text-gray-500">
                    <tr>
                        ...
                        <th class="text-right px-5 py-3 font-medium">Diterima</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @php
                        $outstanding = $ar->total_amount - $ar->paid_amount;
                        $isOverdue = $ar->status !== 'paid' && $ar->due_date && $ar->due_date->isPast();
                    @endphp
                    <tr class="hover:bg-gray-50/50 {{ $isOverdue ? 'bg-red-50/30' : '' }}">
                        <td class="px-5 py-3 text-gray-700 font-medium">{{ $ar->customer->name ?? '-' }}</td>
                        <td class="px-5 py-3 text-gray-500 text-xs font-mono">{{ $ar->invoice_number ?? '-' }}</td>
                        <td class="px-5 py-3 text-gray-600">{{ Str::limit($ar->description, 35) }}</td>
                        <td class="px-5 py-3 text-right text-gray-800">Rp{{ number_format($ar->total_amount, 0, ',', '.') }}</td>
                        <td class="px-5 py-3 text-right text-green-600">Rp{{ number_format($ar->paid_amount, 0, ',', '.') }}</td>
                        ...
                        <td class="px-5 py-3 text-center {{ $isOverdue ? 'text-red-600 font-semibold' : 'text-gray-500' }}">
                            {{ $ar->due_date ? $ar->due_date->format('d/m/Y') : '-' }}
                        ...
                        <td class="px-5 py-3 text-center">
                            ...
                            @elseif ($ar->status === 'partially_paid')
                        ...
                        <td class="px-5 py-3 text-center">
                            @if ($ar->status !== 'paid')
                                <button @click="payModal = true; payId = {{ $ar->id }}; payMax = {{ $outstanding }}; payDesc = '{{ addslashes($ar->customer->name ?? '') }} - {{ addslashes(Str::limit($ar->description, 30)) }}'"
                    ...
                    @empty
                    <tr><td colspan="9" class="px-5 py-8 text-center text-gray-400">Tidak ada data piutang</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="px-5 py-3 border-t border-gray-100">{{ $receivables->links() }}</div>
    </div>
    ...
    <div x-show="payModal" x-transition class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" style="display:none">
        <div @click.outside="payModal = false" class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4">
            <h3 class="text-lg font-semibold text-gray-800 mb-1">Terima Pembayaran Piutang</h3>
            <p class="text-sm text-gray-500 mb-4" x-text="payDesc"></p>

            <form method="POST" :action="'{{ route('manager.finance.ar.pay', '') }}/' + payId">
                @csrf
                <div class="space-y-3">
                    <div>
                    ...
                    <div>
                        <label class="text-sm font-medium text-gray-700">Tanggal Terima</label>
                ...
                <div class="flex justify-end gap-2 mt-5">
                    ...
                    <button type="submit" class="px-5 py-2 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition">Terima Pembayaran</button>
                </div>
            </form>
        </div>
    </div>
