<div x-data="sidebarOffcanvas()" x-init="init()" class="flex bg-indigo-50">
  <nav x-bind:class="sidebarClass()" class="overflow-hidden flex flex-col border-none shadow-xl">
    ...
    <div x-data="{ selected: '{{ $categoryName }}' }" class="flex-1 overflow-y-auto space-y-1 p-3">
      ...
      <!-- All Products Button -->
      <a :href="'/kasir/products?category=All Products'" class="relative flex h-15 w-full items-center rounded-md transition-colors ">
      ...
      @foreach ($categories as $category)
          <a :href="'/kasir/products?category=' + '{{ $category->category_name }}'" class="relative flex h-15 w-full items-center rounded-md transition-colors">
              <button type="button" 
